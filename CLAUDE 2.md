...
11. **`absoluteScore`-flag** (i `INDICATORS`, `shared.ts`) - markerer en social indikator hvis ratio er en absolut score (fx `100 − fossil%`), ikke relativ til landsgennemsnit. Sådanne indikatorer omskaleres IKKE af baseline-toggle (avg/top10/gruppe) - `computeTop10Ratios`/`computeGroupRatios` springer dem over og beholder værdien. Pt. kun `bolig_fossil`.

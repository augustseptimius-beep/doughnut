...
// Import ecological indicators for ratio parsing
// These are defined in fetch_doughnut_data.py and their ids must match
const ECOLOGICAL_IDS = [
  "co2_per_capita",
  "co2_energy",
  "co2_transport",
  "ve_share",
...
export function loadData(): KommuneData[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Ecological indicators (Klimaregnskabet + Energi Data Service)
    for (const id of ECOLOGICAL_IDS) {
      const key = `${id}_ratio`;
      const val = row[key];
      ratios[id] = val && val !== "" ? parseFloat(val) : null;
    }

    const socialAvg = row["social_avg"];
    const ecologicalAvg = row["ecological_avg"];
    ...
    data.push({
      ...
      ecological_avg: ecologicalAvg && ecologicalAvg !== "" ? parseFloat(ecologicalAvg) : null,
      overall_avg: overallAvg && overallAvg !== "" ? parseFloat(overallAvg) : null,
    });
  }

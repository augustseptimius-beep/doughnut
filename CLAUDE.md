...
30. **Kør `python3 scripts/tjek_konsistens.py` før du committer en ændring i konfigurationen** (`shared.ts`, `build_master_csv.py`, `build_trends_csv.py`, `data.ts`, metode-siden). Ren diagnose, samme princip som `tjek_robusthed.py` (punkt 19), men for koden i stedet for datapipelinen. Krydstjekker at samme oplysning - retning (`inverse` vs. `OP_ER_GODT`), øko-fortegn (`lowerIsBetter` vs. faktisk korrelation i master), id-mængder, DST-tabeller, antal indikatorer i metodeteksten, `AVERAGE_DIMENSIONS` - stemmer alle steder den er skrevet. To gennemgange (sep. 2026) fandt gentagne uoverensstemmelser af præcis denne slags; ingen af dem gav en synlig fejl, de viste bare et forkert tal. **Køres automatisk:** `auto_build_master()` printer altid resultatet efter en fetch (rejser aldrig, ligesom resten af den funktion), men `python3 scripts/build_master_csv.py` kørt direkte afbryder med exit 1 hvis der er fejl - det er her det skal stoppes, før commit. Tjekket er skrøbeligt over for større omskrivninger af shared.ts/metode-siden (regex, ikke en rigtig parser) - se scriptets docstring.

31. **Dataår i UI'et beregnes fra master-CSV'en, ikke fra hårdkodede felter (fra sep. 2026).** Footeren (`DOUGHNUT_EDITION_YEAR`/`DOUGHNUT_DEFAULT_DATA_YEAR`), metode-sidens "Datatidspunkt" pr. kategori/dimension og kommunesidens "Data: åååå" pr. indikator kom tidligere fra tre uafhængigt vedligeholdte steder (shared.ts, metode/page.tsx, layout.tsx) og drev fra master-CSV'en og fra hinanden hver gang en indikator blev opdateret (sep. 2026: ca. 25 forkerte dataYear-felter + en footer der stod et helt år forkert). `webapp/lib/data.ts` beregner dem nu direkte fra `master_indicators.csv`'s `data_year`-kolonne: `getDimensionDataYears()` grupperer på `dimension`-feltet (samme id som SOCIAL_CATEGORIES/ECOLOGICAL_DIMENSIONS), `getIndicatorDataYears()` giver ét år pr. social indikator, og `getDoughnutEdition()` udleder footer-årstallene som *mode* (det hyppigste år blandt de sociale indikatorer), ikke max - ellers ville et enkelt fremadrettet registreringsår (fx `bolig_fossil` på BYGB40, mærket 2026) trække editionen et år frem uden grund. `INDICATORS[].dataYear` i shared.ts og `MethodInfo.dataYear` findes ikke længere som primær kilde: førstnævnte er bevaret som fallback-felt i `ScoreBars.tsx` (bruges kun hvis en indikator mangler helt i master), sidstnævnte er fjernet fra metode-siden. **Regel: ret aldrig et forkert dataår ved at rette et hårdkodet felt - roden er altid `data/data_years.json` eller `data_year`-værdien i det pågældende fetch-script.**

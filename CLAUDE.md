...
- **Omfang:** 13 sociale kategorier, 7 økologiske dimensioner, 63 scorede indikatorer (48 sociale + 15 økologiske sub-indikatorer) og 11 kontekst-indikatorer.
...
│   ├── fetch_kystrisiko.py        ← forventet årlig skade fra havet i 2070 (Kystplanlægger, punkt 40)
...
- **Netlify bygger kun ved ændringer i `webapp/`, `data/` eller `netlify.toml`** (`ignore` i `netlify.toml`, fra sep. 2026). Uden den regel sprang Netlify buildet over, når intet under base-mappen `webapp/` var ændret, så en commit der kun opdaterede masterfilen, kom aldrig ud på sitet. Læser webappen en dag filer andre steder fra, skal stien med i `ignore`.
...
Alle kommuner har et sæt **ratios** hvor `100 = niveau med landsgennemsnit` (sociale) eller `100 = på planetens grænse` (økologiske). For sociale indikatorer er højere bedre, for økologiske er lavere bedre. Inverse indikatorer (Gini, kriminalitet, o.l.) vendes af `build_master_csv.py`, så master altid har høj = godt for sociale (punkt 4).
...
Alle multi-indikator-dimensioner bruger **worst-of** (max ratio): overskrides bare én sub-grænse, er hele dimensionen overskredet. Det er bevidst planetary-boundary-logik, ikke gennemsnit. **Eneste undtagelse er `forurening`, som bruger gennemsnit**, fordi dens fire indikatorer måler vidt forskellige forureningstyper. `vand` og `arealanvendelse` er single-indikator.

**Økologiske andele hvor højere er bedre** (natur, genanvendelse, vandområder i god tilstand) har `formula: "komplement"`: ratio = (100 − andel)/(100 − mål) × 100, altså den manglende andel mod det målet tillader. Ingen økologisk indikator har længere et loft (punkt 39).
...
1. Skriv/udvid fetch-script med kolonner `kommune_kode, <id>_raw` og, hvis indikatoren måles mod et landstal, `<id>_ref` (landstallet, samme værdi i alle rækker). Ratioen beregnes af `build_master_csv.py`, ikke af scriptet (punkt 4 og 33). Tilføj `auto_build_master()`-blok (se eksisterende scripts som skabelon).
...
- Bevidst enkel CSV-parsing (split(",")) - brækker hvis felt indeholder komma. Acceptabelt fordi data er kontrolleret, og det håndhæves: registret afviser komma i master-felter, og `build_master_csv.py` fejler hvis et felt indeholder komma (sep. 2026: et komma i en enhed fik `income_gender_gap` til at forsvinde fra sitet uden fejl).
...
9. **Kontekst-indikatorer (vises, scores IKKE)** - data der vises i UI men ikke indgår i nogen score.
    **Regel skærpet sep. 2026: en indikator der KUNNE scores, skal scores eller fjernes. Kontekst er kun til opdelinger af et tal der allerede er scoret.** Sektorfordelingen af CO2 er en opdeling af det territoriale tal; `ctx_fossil_direkte`/`ctx_fossil_via_fjv` er de to halvdele af `bolig_fossil`. De bliver stående, fordi der ikke findes et meningsfuldt mål at score dem imod. `medicin` var derimod en scorbar indikator vi var i tvivl om fortegnet på - den blev fjernet helt, ikke parkeret som kontekst. De sidste kandidater blev afgjort sep. 2026: `ctx_ve_*` og `ctx_energiforbrug` er fjernet (registrets `_fjernet`), `ctx_fritid_*` bliver, fordi de forklarer afgrænsningen af `bolig_fossil`. Defineres i `data/indikatorer.json` med `"category": "context"`, skrives til master med `category="context"`, rutes i `data.ts` til `kommune.rawValues[indicator_id]`, og renderes af en dedikeret komponent (fx `EnergiKontekst`, `KlimaKontekst` i `ScoreBars.tsx`). Bruges til:
    - **Energi:** fjernvarmens brændselsmix (`ctx_fjv_*`, Energistyrelsen EPT `ens.dk/media/7199/download`) og opdelingen af `bolig_fossil` (`ctx_fossil_*`). Begrundelse: fjernvarme er overvejende afbrænding - se metode-siden. VE-kapaciteten (`fetch_ve_kapacitet.py`) hentes stadig, men vises ikke: strømmen går til det nationale net, og tallet blev læst som kommunens egen forsyning.
    - **Klimapåvirkning:** sektorfordeling af territorial udledning (`ctx_klima_landbrug/energi/transport`) fra Klimaregnskabet.dk, samme API-kald som `klimapaavirkning` selv genbruges til (ingen ekstra kald, se `fetch_climate_data.py`). Begrundelse: sektorerne er en opdeling af det allerede scorede territoriale tal, ikke et nyt måltal. Scriptet skriver stadig energiforbrug og VE-selvforsyning til `klimaregnskab_kontekst.csv`, men de står ikke i registret.
    - **Fritidsboliger** (`ctx_fritid_fossil`, `ctx_fritid_andel`, fra `fetch_bolig_fossil.py`). Begrundelse: se punkt 10.
10. **Energi-dimensionen (social)** - scores KUN på `bolig_fossil`, som er kommunens SAMLEDE fossile varmeafhængighed = direkte olie/gas% + (fjernvarme-dækning% × fjernvarmens fossile andel). Scoret mod ABSOLUT mål 0% (ikke landsgennemsnit): `ratio = 100 − samlet_fossil%`, beregnet i `build_master_csv.py` (`formula: "100_minus_raw"` i registret); `fetch_bolig_fossil.py` leverer den samlede fossil-procent. Konsekvens: ingen kommune når grønt (fossilfri varme findes ikke endnu). Fjernvarme-mix og fritidsboliger er kontekst (punkt 9). Fetch-scripts: `fetch_ve_kapacitet.py`, `fetch_fjernvarme_mix.py`. **Kørselsrækkefølge:** `fetch_fjernvarme_mix.py` FØR `fetch_bolig_fossil.py` (sidstnævnte læser fjernvarmens fossil-andel fra førstnævntes CSV). **De 18 kommuner uden egen varmeproduktion får det forsynende nets leverede mix (fra sep. 2026)** fra ENS' "Fjernvarmenet" (`ens.dk/media/8519/download`, status `net` i CSV'en): nettet findes af kommunens anlægsrækker i EPT (`fv_net`), Storkøbenhavns net (2) vælges hvis det er blandt dem, og Dragør, Herlev, Rødovre og Vallensbæk er sat til net 2 i hånden (`NET_MANUELT`). Kun Stevns har intet net og bruger TJ-vægtet landssnit. Fossil er kul + olie + ledningsgas, ikke nettets CO2-faktor (som regner biomasse som nul). Scriptet stopper, hvis net 2 ikke ligger mellem 3 og 12 % fossil - så har ENS ændret filen.
...
17. **11 indikatorer fik pil aug. 2026** (hospital_long, housing_no_wc/no_bath (fjernet sep. 2026), voter_turnout, kultur_spending, civil_society, low_income (fjernet sep. 2026), exam_grade, high_absence, wellbeing, youth_education) - se `fetch_trend_history.py`. **14 af de 63 scorede mangler stadig** og har det bevidst (sep. 2026): ingen tidsserie hos kilden (bolig_fossil, vejr_skader, kystrisiko, forbrug_co2, luftkvalitet_no2/pm25, bio_*, naer_kystvand, overfladevand, nitrat, pesticider), køreplanarkivet går kun tilbage til december 2025 (public_transport, se punkt 38), eller begge år indgår i målet (sport_tilskuer). **apprenticeship (EUD/PRAK/SØG) er pensioneret (aug. 2026)**, fordi UVM ikke længere kender nøgletallet (se registrets `_fjernet`). Skal lærepladser med igen, er praktikpladsgraden det rigtige måltal. `voter_turnout`s kilde stod forkert som KVBPCT i `build_master_csv.py` (den tabel har slet ingen kommune-opdeling) - den rigtige er LABY08, som `fetch_democracy_data.py` allerede brugte korrekt.
18. **En social indikator uden kategori er en fejl (fra sep. 2026).** `housing_no_wc`/`housing_no_bath` stod i master med `dimension=bolig` uden at stå i Bolig-kategoriens `indicators`. Da de fik pil aug. 2026, begyndte Bolig-pilen at gennemsnitte indikatorer ved siden af et tal beregnet på færre - og de usynlige dominerede (`housing_no_bath` faldt ~39% på landsplan). De er nu fjernet (registrets `_fjernet`), og `tjek_konsistens.py` melder en social indikator uden kategori som fejl. `indikatorregister.ikke_scoret()` holder stadig en sådan ude af kategoriens pil, som sikkerhedsnet. Kontrol: kategoriens tooltip ("gennemsnit af N indikatorer") skal matche "N/N indikatorer" på bjælken, MEDMINDRE forskellen skyldes manglende tidsserie på en indikator der faktisk scores (fx Fællesskab 4 af 5, hvor `sport_tilskuer` ingen pil har - det er korrekt).
...
19. **Datapipelinens robusthed - kør `python3 scripts/tjek_robusthed.py` før en dataopdatering.** Ren diagnose, skriver ingen filer. Den tjekker fem fejlklasser vi alle er faldet i (aug. 2026), og som har samme signatur: platformen ser levende ud, men tallet er frosset, og INTET fejler højlydt.
    ...
    - **Døde tabeller.** DST markerer tabeller inaktive uden at fjerne dem. HFUDD10 stoppede i 2019 (`education` viste 2019-tal mærket 2023), BIB1 er afløst af BIB3A, og NEET1 (sidste år 2023) af NEET3 med alder 16-24 (sep. 2026). Tjek `active`-flaget - der findes næsten altid en afløser, og den har typisk samme tal tilbageberegnet.
    - **Hårdkodede årstal.** Den enkeltårsag der holdt flest indikatorer forældede. Brug `scripts/dst_aar.py`: `seneste_aar()`, `seneste_periode()` (5-års-intervaller som HISBK "2021:2025"), `seneste_kvartal()` (FOLK1A SKAL blive på K1, ellers skifter opgørelsesdatoen umærkeligt; tal pr. indbygger bruger `dst.folketal()`, 1. januar i tallets eget år, se punkt 37) og `hele_aar_kvartaler()` (STRAF11 kræver fire hele kvartaler).
...
21. **Nitrat og pesticider kommer nu fra GEUS Jupiter (aug. 2026), ikke fra PDF-udtræk.** `scripts/fetch_grundvand_jupiter.py` afløser `fetch_nitrat_data.py` og `fetch_pesticider_data.py`, som begge indeholdt hårdkodede tal. **Den gamle nitrat-kilde gav 80 af 98 kommuner den IDENTISKE pladsholderværdi 3,7 mg/L** - kun Greenpeaces top-20 havde rigtige tal. Derfor skiftede 36 kommuner farve på Forurening ved omlægningen; det er korrektioner, ikke støj. Fire ting man skal kende:
    - **Kun aktive vandværker (`aktiv_num=1`, fra sep. 2026).** Jupiter har også nedlagte værker med. Med dem stod Thisted med 5,2 mg/L nitrat mod 14,5 for de aktive, og korrelationen med Greenpeaces top-20 steg fra 0,80 til 0,93, da de blev taget ud.
    ...
    - **Tallet er beslægtet med Greenpeaces, ikke identisk.** Vi vægter efter tilladt indvindingsmængde; Greenpeace/Schullehner kobler til faktiske forsyningsområder. Aalborg (20,6 mod 20,7) og Thisted (14,5 mod 13,9) rammer tæt; tidligere lå Thisted på 5,2, men det skyldtes de nedlagte vandværker, ikke metoden.
    - **Pesticider er andelen af aktive vandværker med FUND i seneste analyse (fra sep. 2026), udglattet med empirisk Bayes.** Indtil da talte indikatoren "over kravværdien", men regnede "Aktuelt fund og tidl. over kravværdi" med som over, selv om seneste analyse er under (Gentofte stod med 100%). Kun 33 aktive værker er aktuelt over kravværdien, for få til at skelne kommuner. Beta-binomial-udglatningen (Kleinman 1973) trækker kommuner med få værker mod landsandelen; prioren svarer til ca. 12 vandværkers vægt.
...
33. **Referencen bag hver ratio står i registret og i master (fra sep. 2026).** `reference.type` er `maal` (fast mål), `kommunegennemsnit` (uvægtet, beregnet ved build) eller `landstal` (fetch-scriptets tal - `definition` siger hvordan det er fundet). **Landsgennemsnit betyder Danmark som helhed:** de 98 kommuner samlet, vægtet med indikatorens egen nævner, ikke et uvægtet snit og ikke en hele-landet-række med tal uden kommune (kriminalitet: 8,7% uden gerningskommune). Tal pr. indbygger danner landstallet i `dst.pr_indbygger()`. Ingen indikator bruger længere et uvægtet kommunegennemsnit: de fire UVM-indikatorer vægtes fra sep. 2026 med folkeskoleelever efter bopælskommune (DST UDDAKT20, `fetch_udvidelse_data.py --kun-landstal` kræver ingen UVM-nøgle). Typen `kommunegennemsnit` findes stadig i registret til en fremtidig indikator uden nævner. Master har en ny sidste kolonne, `reference`, med den værdi ratioen er målt mod. **Fetch-scripterne skriver landstallet i `reference.col` (`<id>_ref`) fra sep. 2026**, og build bruger det direkte. For en CSV der ikke er hentet siden, rekonstrueres det stadig fra scriptets ratio (median af `raw×100/ratio`, afrundet til færrest decimaler der stadig rammer scriptets ratios); build-outputtet siger så "landstal, rekonstrueret". Et nyt script skal skrive `<id>_ref`. To ting fulgte med omlægningen: (1) 7 rækker for kvælstof/fosfor fra spildevand (Frederiksberg, Fanø, Rødovre, Herlev, Ærø) stod med ratio 66,67 uden råværdi - en artefakt af at 0 udledning blev til topscore 150 på den inverterede skala og derefter 10000/150. De er væk. (2) `scripts/rapport_dataaendringer.py` sammenligner ny master med den committede (rækker, ratios, råværdispring over 25%, farveskift, dataår) og køres automatisk efter hvert build. Læs den før commit - det var her Vejen-fejlen ville være blevet fanget.

34. **Webappen nægter at bygge fra et master der ikke passer med registret (fra sep. 2026).** `validerMaster()` i `webapp/lib/data.ts` kører ved hvert build (også på Netlify) og i dev. Den stopper buildet hvis master indeholder et id registret ikke kender, hvis en scoret indikator mangler i master, eller hvis en ratio ikke passer med `råværdi` og `reference` efter formlen (en kontrol-kopi af `beregn_ratio()`, tolerance 0,011). Typisk årsag: `data/indikatorer.json` er rettet uden at `python3 scripts/build_master_csv.py` er kørt bagefter. Netlify beholder så den forrige version online, i stedet for at deploye tal der modsiger metoden. Første kørsel fandt straks at `income_gender_gap` manglede på sitet (se CHANGELOG 22. sep.). Ændrer du formlen i `build_master_csv.py`, skal kontrol-kopien i `data.ts` følge med - ellers fejler buildet, hvilket er meningen.

35. **Skriv aldrig et landstal eller en dækning som tal i en tekst - brug en pladsholder (fra sep. 2026).** Registrets tekster (beskrivelser, grænser, begrundelser) og metodesidens `SOCIAL_METHODS`/`ECO_METHODS` skriver fx `{ref:pesticider:1}%`, `{daekning:sport_tilskuer} af {kommuner}`, `{mangler:vandindvinding}` og `{aar:areal_antropiseret}`. `udfyldTal()` i `shared.ts` udfylder dem fra `data/noegletal.json`, som `build_master_csv.py` skriver sammen med master. Håndskrevne tal som "9,2%", "72,9 m³" og "76 af 98" drev tidligere ved hver dataopdatering. En pladsholder der peger på en ukendt indikator, fanges af `tjek_konsistens.py` og stopper webappens build. Tal der ikke findes i master (fx fjernvarmens landsgennemsnit ~13%, som er kontekst uden reference), står stadig som tekst og skal rettes i hånden.
...
37. **Pilen og scoren er samme tal - én funktion pr. indikator (fra sep. 2026).** Indikatorer med en beregning (tal pr. indbygger, andele, treårsgennemsnit) hentes af `serie_<id>(perioder)` i fetch-scriptet. Scoren kalder den med det nyeste år, `fetch_trend_history.py` med alle år (listen `SAMME_SOM_SCOREN`). Før havde tidsserien sin egen definition, og for 14 indikatorer beskrev pilen et andet tal end scoren, fx klassekvotient kun i folkeskolen mod scorens alle skoletyper, ubeboede boliger inkl. fritidshuse (Thisted 24% mod 11,5%) og Klimaregnskabets to "Samlet"-rækker lagt sammen i stedet for den største (Læsø -5,2 mod 4,7). Tre konventioner gælder begge steder: tal pr. indbygger deles med folketallet 1. januar i tallets eget år (`dst.folketal()`), perioder og skoleår mærkes med slutåret (`dst_aar.aarstal()`), og treårsgennemsnit er gennemsnittet af de tre års rater. **`tjek_konsistens.py` sammenligner serie og score** og melder fejl, når over 10% af kommunerne afviger over 1%. Tilføjer du en indikator med pil, så skriv en `serie_`-funktion og sæt den i `SAMME_SOM_SCOREN` frem for en ny tekstsøgning i `SIMPLE`. Indikatorer der stadig hentes generisk i `SIMPLE` (fx gini, beskæftigelse), passer i dag med scoren og holdes der, så længe tjekket er grønt. Fetch-scripterne skriver samtidig selv landstallet i `<id>_ref`, så build ikke længere skal rekonstruere det (punkt 33).
38. **`public_transport` beregnes af os pr. kommune (fra sep. 2026).** DST's LABY49 findes kun for fem kommunegrupper, så alle kommuner i en gruppe fik samme tal, og i kommunegruppe-baselinen fik alle 98 præcis 100. `fetch_offentlig_transport.py` genskaber DST's metode (verdensmål 11.2.1) med Rejseplanens GTFS, DAR-adresser (DAWA) og Eurostats befolkningsgrid 2021. Metode og validering: `docs/offentlig-transport-genskabt.md`. Fem ting man skal kende:
    - **Frekvensen er SUMMEN af afgange fra alle stop inden for rækkevidden** - en bus tæller ved hvert stop den passerer. "Bedste stoppested" og "unikke ture" blev testet og rammer DST systematisk for lavt. Skift ikke reglen uden at genvalidere.
...
39. **Økologisk gennemgang sep. 2026 - hvad der ændrede sig og hvorfor.** Se `docs/oekologisk-gennemgang-sep-2026.md` for svagheder, kilder og fravalg. Det vigtigste at vide ved drift:
...
40. **Klimatilpasning måler restrisiko (fra sep. 2026).** Kategorien skal vise, hvor robust kommunen er over for klimaforandringerne, når det der allerede er gjort, er regnet med: lav score = stor risiko. `vejr_skader` er faktiske skader (reagerer på tiltag), `kystrisiko` er Kystdirektoratets forventede årlige skade fra havet i 2070 (RCP8.5) i kr. pr. indbygger. Fire ting at vide om `kystrisiko`:
...
- **`docs/artikler/`** - to metodeartikler (.docx) til offentligheden om den sociale og den økologiske ring, med følsomhedstest og svagheder. Tallene er et øjebliksbillede af data 26. sep. 2026 og genskabes med `analyse_artikler.py`. De er ikke lagt på platformen endnu (afventer godkendelse).

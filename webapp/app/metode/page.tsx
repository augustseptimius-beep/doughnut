...
const SOCIAL_METHODS: Record<string, MethodInfo> = {
  ...
  uddannelse: {
    ...
    limitations: "Karaktergennemsnit afspejler ikke kun skolekvalitet men også socioøkonomisk baggrund. Klassekvotienter og normering fanger kvantitative mål, ikke undervisningskvalitet. UVM-tallene er fra forskellige skoleår (karakterer {aar:exam_grade}, fravær {aar:high_absence}, trivsel {aar:wellbeing}, forventet ungdomsuddannelse {aar:youth_education}). UVM udgiver ikke et landstal pr. indikator, så landsgennemsnittet for de fire UVM-indikatorer beregnes af platformen og vægtes med antal folkeskoleelever efter bopælskommune (DST UDDAKT20). Indtil september 2026 var det et uvægtet gennemsnit af kommunerne, hvor små kommuner vejede lige så meget som store.",
  },
  velfaerd: {
    id: "velfaerd",
  ...
  energi: {
    ...
    limitations: "Datagrundlaget har forskellig alder: opvarmet areal (BYGB40) er fra 2026, fjernvarmemix fra 2024. Kun fossil afhængighed indgår i scoren. (1) Lokal VE-kapacitet er bevidst holdt ude: en kommune kan have mange vindmøller OG mange oliefyr, og strømmen går til det nationale net - ikke til kommunens egne husstande. At gennemsnitte de to ville udvande scoren og antyde at vindmøller kompenserer for oliefyr. Af samme grund vises VE-kapaciteten heller ikke som kontekst: et tal pr. kommune bliver let læst som kommunens egen grønne forsyning. (2) Kun den FOSSILE del af fjernvarmen tælles med - biomasse og affald regnes hverken som grønt eller sort (det undgår det omdiskuterede værdivalg om biomasse). Fjernvarmens fulde mix vises som kontekst. (3) Fjernvarmens fossilandel stammer fra EPT, der opgøres ved produktionsstedet. De 18 kommuner uden egen varmeproduktion (fx hovedstadskommunerne) får i stedet fordelingen af den varme, der leveres til det net, der forsyner dem (Energistyrelsen, Fjernvarmenet 2024) - fx 6,2% fossil for Storkøbenhavns Fjernvarme. Kun Stevns, hvor nettet ikke kan findes, bruger det TJ-vægtede landsgennemsnit (~13%). (4) Direkte fossil og fjernvarme-dækning er begge fra BYGB40 på arealbasis, så de er konsistente. (5) FRITIDSBOLIGER er bevidst holdt ude af scoren og vises kun som kontekst. Sommerhuse er typisk elopvarmede og har markant lavere fossilandel end helårsboliger (median ca. 7% mod ca. 20%). Hvis de indgik, ville sommerhuskommuner som Odsherred, Gribskov og Fanø fremstå kunstigt bedre på et mål der handler om HUSSTANDES varmeregninger - og sommerhusene ejes typisk af folk fra andre kommuner. (6) Erhvervs- og avlsbygninger, garager og udhuse indgår ikke: indikatoren måler boliger, ikke kommunens samlede bygningsmasse.",
  },
};
